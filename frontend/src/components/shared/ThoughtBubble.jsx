 ...
 *
 * Each bubble first shows a "typing" dots indicator for ~700ms so the
 * thought feels like Shanaya is forming it in real time, then the dots
 * crossfade out and the actual text crossfades in. The entry uses spring
 * physics so the bubble has a gentle bounce — not a hard pop.
 *
 * `position` is one of:
  ...
  return (
    <div className={`pointer-events-none flex flex-col gap-3 ${align}`} aria-live="polite">
      <AnimatePresence initial={false} mode="popLayout">
        {visible.map((b, i) => (
          <Bubble key={`${b.text}-${i}`} bubble={b} dotSide={dotSide} />
        ))}
      </AnimatePresence>
    </div>
  );
...
function Bubble({ bubble, dotSide }) {
  /* Brief typing indicator before the bubble's text appears. Make every
   * bubble feel like Shanaya is forming the thought in real time, instead
   * of the whole sentence popping in fully written. */
  const [phase, setPhase] = useState('typing'); // 'typing' | 'said'
  useEffect(() => {
    const t = setTimeout(() => setPhase('said'), 700);
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <div className="relative min-w-[7rem] max-w-[20rem] rounded-[1.6rem] bg-white px-4 py-3 text-[15px] leading-snug text-ink-900 shadow-xl ring-1 ring-ink-300/15 sm:text-base">
